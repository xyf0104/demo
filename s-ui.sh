...
update() {
    confirm "此操作将强制重装最新版本，数据不会丢失。是否继续？" "n"
    if [[ $? != 0 ]]; then
        LOGE "已取消"
        if [[ $# == 0 ]]; then
            before_show_menu
...
custom_version() {
    ...
    download_link="https://raw.githubusercontent.com/xyf0104/demo/main/s-ui-install.sh"
...
uninstall() {
    ...
    echo ""
    echo -e "卸载成功！如需删除管理脚本，退出后运行 ${green}rm /usr/local/s-ui -f${plain} 即可删除。"
    echo ""
...
set_setting() {
    echo -e "Enter the ${yellow}面板端口${plain} (留空使用现有/默认值):"
    read config_port
    echo -e "Enter the ${yellow}面板路径${plain} (留空使用现有/默认值):"
    read config_path

    echo -e "Enter the ${yellow}订阅端口${plain} (留空使用现有/默认值):"
    read config_subPort
    echo -e "Enter the ${yellow}订阅路径${plain} (留空使用现有/默认值):" 
    read config_subPath
...
start() {
    ...
    if [[ $? == 0 ]]; then
        echo ""
        LOGI -e "${1} 已在运行，无需再次启动。如需重启请选择重启选项"
    else
        systemctl start $1
        sleep 2
        check_status $1
...
disable() {
    systemctl disable $1
    if [[ $? == 0 ]]; then
        LOGI "Autostart ${1} 已取消 successfully"
    else
        LOGE "取消开机自启失败"
    fi
...
update_shell() {
    wget -O /usr/bin/s-ui -N --no-check-certificate https://github.com/xyf0104/demo/raw/main/s-ui.sh
    if [[ $? != 0 ]]; then
        echo ""
        LOGE "下载脚本失败，请检查服务器是否能连接 GitHub"
        before_show_menu
    else
        chmod +x /usr/bin/s-ui
        LOGI "脚本升级成功，请重新运行" && exit 0
    fi
}
...
enable_bbr() {
    ...
    arch | manjaro | parch)
        ...
        ;;
    *)
        echo -e "${red}不支持的操作系统，请手动安装所需依赖包。${plain}\n"
        exit 1
        ;;
    esac
...
ssl_cert_issue() {
    if ! command -v ~/.acme.sh/acme.sh &>/dev/null; then
        ...
        install_acme
        if [ $? -ne 0 ]; then
            LOGE "正在安装 acme...iled, please check logs"
            exit 1
        fi
    fi
    case "${release}" in
...
ssl_cert_issue_CF() {
    ...
    case $choice in
        1|2)
            ...
            LOGI "3. 已在 Cloudflare 解析到本服务器的域名"
            ...
            if [ $? -eq 0 ]; then
                ...
                CF_AccountEmail=""
                LOGD "请输入 API Key："
                read -p "请输入 Key： " CF_GlobalKey
                LOGD "API Key 已设置为： ${CF_GlobalKey}"

                LOGD "请输入注册邮箱："
                read -p "请输入邮箱： " CF_AccountEmail
                LOGD "注册邮箱已设置为： ${CF_AccountEmail}"

                ~/.acme.sh/acme.sh --set-default-ca --server letsencrypt
                if [ $? -ne 0 ]; then
                    LOGE "设置 Let's Encrypt 为默认 CA 失败，退出..."
                    exit 1
                fi
...
show_usage() {
    ...
    echo -e "s-ui enable       - 开启开机自启"
    echo -e "s-ui disable      - 关闭开机自启"
    echo -e "s-ui log          - 查看 S-UI 日志"
    echo -e "s-ui update       - 更新"
    echo -e "s-ui install      - 安装"
    echo -e "s-ui uninstall    - 卸载"
    echo -e "s-ui help         - 命令行用法"
    echo -e "------------------------------------------"
}

show_menu() {
  echo -e "
  ${green}S-UI 管理面板 ${plain}
————————————————————————————————
  ${green}0.${plain} 退出
————————————————————————————————
  ${green}1.${plain} 安装
